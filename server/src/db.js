/**
 * In-Memory Database for Bantu Cafe
 * Schema conforms to PRD.md (menus, orders, order_items tables)
 */
...
// ─── Menus (mirrors client/src/data/mockMenus.js) ──────────────
export const menus = [
  {
    id: 1,
    category_id: 'coffee',
...
/**
 * Generate unique order code: #BC-101, #BC-102, ...
 */
export function generateOrderCode() {
  const code = `#BC-${String(100 + nextOrderId).padStart(3, '0')}`;
  nextOrderId++;
  return code;
}
...
/**
 * Find an order by its unique order_code string
 */
export function findOrderByCode(orderCode) {
